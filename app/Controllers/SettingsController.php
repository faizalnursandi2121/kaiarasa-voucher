...
class SettingsController extends Controller
{
    ...
    public function update()
    {
        ...
        $data = [
            ...
            'hotspot_name' => $_POST['hotspotname'] ?? '',
            'dns_name' => $_POST['dnsname'] ?? '',
            // Field ini tidak ada di form v2 — pertahankan nilai existing
            // agar update dari modal edit tidak menghapus konfigurasi lama.
            'currency' => $_POST['currency'] ?? ($existingRouter['currency'] ?? ''),
            'reload_interval' => $_POST['areload'] ?? ($existingRouter['reload_interval'] ?? ''),
            'interface' => $_POST['iface'] ?? '',
            'description' => $existingRouter['description'] ?? 'Updated via Remake',
            'quick_access' => isset($_POST['quick_access']) ? 1 : 0,
            'port' => (int) ($_POST['port'] ?? 8728),
            'ssl' => isset($_POST['ssl']) ? 1 : 0,
        ];
