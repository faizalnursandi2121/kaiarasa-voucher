...
class ProfileController extends Controller
{
    ...
    public function store()
    {
        ...
        $logicScript = '';
        if (! empty($validity)) {
            $logicScript = ' :local v "'.$validity.'"; :local u $username; :local c [/ip hotspot user get [find name=$u] comment]; :if ([:len [/sys sch find where name=$u]] = 0) do={ /sys sch add name=$u interval=$v on-event=":do { /ip hotspot user set [find name=$u] disabled=yes } on-error={}; /sys sch remove [find name=$u]"; /ip hotspot user set [find name=$u] comment=("exp: " . $v . " " . $c); }';
        }
