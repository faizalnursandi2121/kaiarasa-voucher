...
$toolbar_html = '
    <div class="input-group md:w-64 z-10">
        ...
        <input type="text" id="global-search" class="form-input-search w-full" placeholder="Search user, mac..." data-i18n="common.table.search_placeholder">
    </div>
                    ...
                    <tr class="table-row-item"
                        ...
                        <td class="text-right text-sm font-medium">
                            <div class="flex justify-end table-actions-reveal">
                                <form action="/<?= htmlspecialchars($session) ?>/hotspot/cookies/remove" method="POST" onsubmit="event.preventDefault(); Kaiarasa.confirm(window.i18n ? window.i18n.t('cookies.remove_cookie') : 'Remove Cookie?', window.i18n ? window.i18n.t('cookies.remove_confirm', {user: '<?= htmlspecialchars($cookie['user'] ?? '') ?>'}) : 'Are you sure you want to remove the cookie for <?= htmlspecialchars($cookie['user'] ?? '') ?>?', window.i18n ? window.i18n.t('common.delete') : 'Remove', window.i18n ? window.i18n.t('common.cancel') : 'Cancel').then(res => { if(res) this.submit(); });">
