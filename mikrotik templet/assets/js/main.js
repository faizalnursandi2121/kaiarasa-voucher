...
window.MivoConfig = {
    // API Configuration
    // MIVO/Kaiarasa Voucher Manager server
    apiBaseUrl: "https://captive-kaiarasa.stg.net.id",

    // Your Mivo Session Name (registered in MIVO dashboard)
    // TODO: ganti dengan nama session router Kaiarasa yang sebenarnya
...
// 2. Global Utilities
// English time unit labels (i18n removed — English only)
const TIME_UNITS = { w: 'Week', d: 'Day', h: 'Hour', m: 'Minute', s: 'Second' };
...
window.formatTime = function(str) {
    ...
    while ((match = regex.exec(normalized)) !== null) {
        const val = match[1];
        const unit = match[2];
        const unitName = TIME_UNITS[unit] || unit;
...
window.formatSeconds = function(seconds) {
    ...
    if (w > 0) parts.push(`${w} ${TIME_UNITS.w}`);
    if (d > 0) parts.push(`${d} ${TIME_UNITS.d}`);
    if (h > 0) parts.push(`${h} ${TIME_UNITS.h}`);
    if (m > 0) parts.push(`${m} ${TIME_UNITS.m}`);
    if (s > 0) parts.push(`${s} ${TIME_UNITS.s}`);
